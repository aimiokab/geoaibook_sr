...
class FlowSolver(torch.nn.Module):
    ...
    def forward_flow_and_score(self, t, x, only_flow=False):
        ...
        if self.separate_score:
            vt, st = self.net(t, x, self.shape, dates=self.dates, low_res=self.low_res, MISR=self.MISR, context=self.context), self.score_net(t, x, self.shape, dates=self.dates, low_res=self.low_res, MISR=self.MISR, context=self.context)
        else:
            vtst = self.net(t, x, self.shape, dates=self.dates, low_res=self.low_res, MISR=self.MISR, context=self.context)
    ...
    def aug_net_(self, t, x):
        return self.aug_net(t, x, shape=self.shape, dates=self.dates, low_res=self.low_res, MISR=self.MISR, context=self.context)
    ...
    def forward_ode_drift(self, t, x, *args, **kwargs):
    ...
    def odeint(self, x0, t_span, shape, dates=None, low_res=None, MISR=False, context=None):
        ...
        self.aug_net = AugmentedVectorField(self.forward_ode_drift, self.augmentations.regs, self.dim)
